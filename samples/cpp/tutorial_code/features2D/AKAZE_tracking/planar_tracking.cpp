...
void Tracker::setFirstFrame(const Mat frame, vector<Point2f> bb, string title, Stats& stats)
{
    ...
    detector->detect(first_frame, first_kp);
    detector->compute(first_frame, first_kp, first_desc);
    stats.keypoints = first_kp.size();
...
int main(int argc, char **argv)
{
    ...
    VideoWriter  video_out(argv[2],
                           video_in.get(CAP_PROP_FOURCC),
                           video_in.get(CAP_PROP_FPS),
                           Size(2 * video_in.get(CAP_PROP_FRAME_WIDTH),
                                2 * video_in.get(CAP_PROP_FRAME_HEIGHT)));
    ...
    int frame_count = video_in.get(CAP_PROP_FRAME_COUNT);
